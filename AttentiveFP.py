...
def model_builder(model_dir, **model_params):
    model = AttentiveFPModel(mode='classification',n_tasks=len(tasks),**model_params)
    return model
...
params_dict={'dropout':[0.1,0.3,0.5],'weight_decay':[0,0.01,0.0001],'learning_rate':[0.1, 0.01, 0.001],
            'graph_feat_size':[50, 100, 200]}
...
model = AttentiveFPModel(mode='classification',
                 ...
                 graph_feat_size = best_hyperparams[3])
...
for x, y, w, id in valid_dataset.itersamples():
    valid_true.append(y)
...
for i in range(num_epochs):
    ...
    y_pre = y_pred[:, 1].squeeze()
    auc_roc_score_test = roc_auc_score(valid_true, y_pre)
