...
def model_builder(model_dir, **model_params):
    model = GraphConvModel(mode='classification',n_tasks=len(tasks),**model_params)
    return model
...
metric = dc.metrics.Metric(dc.metrics.roc_auc_score) #AUC
params_dict={'dropout':[0.3,0.5,0.7],'alpha':[0.2,0.1],'learning_rate':[0.01, 0.001, 0.0001],
            'dense_layer_size':[32, 64, 128],'graph_conv_layers':[(64, 64), (128, 128), (256, 256)]}
...
best, best_hyperparams,all_results= optimizer.hyperparam_search(params_dict,
                                                                ...
                                                                valid_dataset,
                                                                metric)
model = GraphConvModel(mode='classification',
                 n_tasks=len(tasks),
                 ...
                 graph_conv_layers=best_hyperparams[4])
